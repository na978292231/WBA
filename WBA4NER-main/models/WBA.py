...
from modules.htfomer import HTFormerEncoder
...
class WBA(nn.Module):
    def __init__(self, tag_vocab, char_embed, word_embed, num_layers, hidden_size, n_head, feedforward_dim, dropout,
        ...
        self.chars_transformer = TransformerEncoder(num_layers, hidden_size, n_head, feedforward_dim, dropout,
                                                    after_norm=after_norm, attn_type=attn_type,
                                                    scale=scale, attn_dropout=attn_dropout,
                                                    pos_embed=pos_embed, vocab=vocab)
        ...
        self.htformer = HTFormerEncoder(num_layers, hidden_size, n_head, max_seq_len, feedforward_dim, softmax_axis=softmax_axis,
                                       four_pos_fusion=four_pos_fusion, fc_dropout=dropout, attn_dropout=attn_dropout,
                                       q_proj=q_proj, k_proj=k_proj, v_proj=v_proj, r_proj=r_proj, scale=scale, vocab=vocab)
    ...
    def _forward(self, chars, target, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num,head,tail, bigrams=None):
        char_ids = chars
        word_ids = words
        ...
        if self.before:
            ...
            chars = self.chars_transformer(chars, char_ids, seq_len,chars_mask)
        '''
        chars = self.informer(chars, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num, char_ids, word_ids)
        chars.masked_fill_(~(chars_mask.unsqueeze(-1)), 0)
        chars = self.htformer(chars, words, pos_s, pos_e, head, tail, seq_len, lex_num,char_ids, word_ids)
    ...
    def forward(self, chars, target, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num, head,tail,bigrams=None):
        return self._forward(chars, target, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num,head,tail, bigrams)

    def predict(self, chars, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num,head,tail, bigrams=None):
        return self._forward(chars, None, words, pos_s, pos_e, lex_s, lex_e, seq_len, lex_num,head,tail, bigrams=bigrams)
