...
from modules.htrel_pos_embedding import Four_Pos_Fusion_Embedding
from modules.utils import get_embedding, char_lex_len_to_mask


class HTFormerEncoder(nn.Module):
    def __init__(self, num_layers, hidden_size, n_head, max_seq_len, feedforward_dim, softmax_axis=-1, four_pos_fusion= 'ff',
        ...
        for i in range(self.num_layers):
            setattr(self, 'layer_{}'.format(i), InterFormerLayer(hidden_size, n_head, feedforward_dim, softmax_axis=softmax_axis,
                                                             ...
                                                             k_proj=k_proj, v_proj=v_proj, r_proj=r_proj, max_seq_len=max_seq_len,scale=scale, vocab=vocab))


    def forward(self, chars, words, pos_s, pos_e, head,tail, seq_len, lex_num, char_ids=None, word_ids=None):
        rel_pos_embedding = self.four_pos_fusion_embedding(seq_len, lex_num, pos_s, pos_e, head,tail)
        output = chars
        for i in range(self.num_layers):
...
class InterFormerLayer(nn.Module):
    def __init__(self, hidden_size, n_head, feedforward_dim, softmax_axis=-1, fc_dropout=0.3, attn_dropout=0,
                 q_proj=True, k_proj=True, v_proj=True, r_proj=True,max_seq_len=None, scale=False, vocab=None):
        super().__init__()

        self.attn = InterAttention(hidden_size, n_head, attn_dropout=attn_dropout, softmax_axis=softmax_axis,
                 q_proj=q_proj, k_proj=k_proj, v_proj=v_proj, r_proj=r_proj, max_seq_len=max_seq_len,scale=scale, vocab=vocab)
...
class InterAttention(nn.Module):
    def __init__(self, hidden_size, n_head, attn_dropout=0, softmax_axis=-1,
        ...
        self.times = 0
        self.count=0
        get_embed=get_embedding(max_seq_len,hidden_size)
        self.get_embed = nn.Parameter(get_embed, requires_grad=False)
        self.pos_proj = nn.Sequential(nn.Linear(hidden_size * 3,hidden_size),
                                          nn.ReLU(inplace=True),
                                          nn.Linear(hidden_size,hidden_size))
    def forward(self, chars, words, seq_len, lex_num, rel_pos_embedding,char_ids, word_ids):
        batch, max_char_len, _ = chars.size()
        _, max_word_len, _ = chars.size()
        ...
        if self.times % 1350 == 0:
            ...
            def attention_visualization(head_i,count):
                ...
                data = att_score.cpu().detach().numpy()[index_][head_i][:seq_length, :seq_length].T
                #fitlog.add_hyper_in_file(data)
                np.savetxt("./plt/wfile.log", data)
                ...
                if not char_ids is None and not word_ids  is None:
                    ...
                    plt.savefig('./plt/bw{}a{}.png'.format(count,head_i))
                    plt.show()
