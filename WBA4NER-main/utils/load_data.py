...
def load_ner(data_path, unigram_embedding_path, bigram_embedding_path, index_token, char_min_freq,
    ...
    loader = ConllLoader(headers=['chars','head','tail','target'],indexes=[0, 1, 2,5])
    ...
    for k, v in datasets.items():
        ...
        v.apply_field(lambda head:head,field_name='head',new_field_name='head')
        v.apply_field(lambda tail:tail,field_name='tail',new_field_name='tail')
...
def equip_chinese_ner_with_lexicon(datasets, vocabs, embeddings, w_list, word_embedding_path=None,
    ...
    for k, v in datasets.items():
        ...
        v.apply(get_head,new_field_name='head')
        v.apply(get_tail, new_field_name='tail')
